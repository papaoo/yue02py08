...
def evaluate_single_model(model_type, model_path, config):
    """评估单个模型"""
    # 设置设备
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets) in enumerate(tqdm(test_loader, desc=f"评估 {model_type}")):
    ...
    print(f"评估结果已保存至: {os.path.join(results_dir, 'evaluation_results.json')}")
...
def evaluate_all_models():
    ...
    for model_config in models:
        ...
        if os.path.exists(model_path):
            try:
                results = evaluate_single_model(model_type, model_path, config)
    ...
    # 保存所有模型的评估结果对比
    comparison_path = os.path.join(config['output_dir'], 'models_evaluation_comparison.json')
    with open(comparison_path, 'w') as f:
        json.dump(all_results, f, indent=2)
    
    print(f"\n===== 所有模型评估完成 =====")
    print(f"评估结果对比已保存至: {comparison_path}")
    
    # 打印对比结果
    print("\n模型性能对比:")
    print("-" * 80)
    print(f"{'模型类型':<15} {'成功率':<10} {'平均路径长度':<15} {'平均平滑度':<15} {'平均推理时间(ms)':<20}")
    print("-" * 80)
    for model_type, results in all_results.items():
        if 'error' not in results:
            print(f"{model_type:<15} {results['success_rate']:<10.4f} {results['avg_path_length']:<15.4f} {results['avg_smoothness']:<15.4f} {results['avg_inference_time_ms']:<20.4f}")
        else:
            print(f"{model_type:<15} {'错误':<10} {'错误':<15} {'错误':<15} {'错误':<20}")
    print("-" * 80)
